...
namespace libsignalservice.websocket
{
    internal class WebSocketWrapper : IDisposable
    {
        ...
        public event Action<byte[], WebSocketWrapper> MessageReceived;
        ...
        public async Task Connect(string uri, string protocol = null, IDictionary<string, string> headers = null)
        {
            ...
            Debug.WriteLine("WebSocketWrapper::connectAsync " + uri+ " cancelToken "+_cancellationToken);
            try
            {
        ...
        public async Task SendMessage(byte[] message)
        {
            ...
            try
            {
                ...
                {
                    await _ws.SendAsync(new ArraySegment<byte>(message, 0, message.Length), WebSocketMessageType.Binary, true, _cancellationToken);
                }
            }
            catch (Exception ex)
            {
